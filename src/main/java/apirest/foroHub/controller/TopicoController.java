...
import apirest.foroHub.domain.topic.TopicoService;
import apirest.foroHub.domain.topic.dto.ListarTopicosDTO;
...
public class TopicoController {
    ...
    public ResponseEntity<Page<ListarTopicosDTO>>  listarTopicos(@PageableDefault(size = 10) Pageable paged){
        return ResponseEntity.ok(topicoRepository.findByActivoTrue(paged).map(ListarTopicosDTO::new));
    }
    ...
    public ResponseEntity topicoActualizado (@RequestBody @Valid TopicoActualizadoDTO topicoActualizadoDTO){
        ...
        return ResponseEntity.ok(new RespuestaTopicoDTO(topico.getId(),
                topico.getTitulo(),topico.getMensaje(), topico.getStatus(),topico.getAutor().getId(),
